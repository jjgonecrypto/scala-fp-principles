...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toList.groupBy((c: Char) => c.toLower).map { case (c: Char, lc: List[Char]) => (c, lc.size)}.toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match {
      case Nil => List(List())
      case (c: Char, n: Int) :: tail => {
        (for {
          i <- 1 to n
          rest <- combinations(tail)
        } yield (c, i) :: rest) ++ combinations(tail)
      }.toList
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (for {
      (c, list) <- (x ++ y).groupBy { case(a:Char, i:Int) => a }.toList.sortBy(_._1)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def loop(acc: Occurrences): List[Sentence] = {
        if (acc.isEmpty) List(List())
          ...
          for {
            occurrence <- combinations(acc)
            if (dictionaryByOccurrences.contains(occurrence))
            word <- dictionaryByOccurrences.get(occurrence).get
            remainder <- loop(subtract(acc, occurrence))
